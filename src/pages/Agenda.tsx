...
import { mockAppointments, mockEmployees } from '@/lib/mock-data';
...
const statusColors = {
  pending: 'bg-warning/20 border-l-warning text-foreground',
...
const Agenda = () => {
  ...
  const getAppointmentsForDate = (date: Date) => {
    return mockAppointments.filter(
      (a) => a.date.toDateString() === date.toDateString() && a.status !== 'cancelled'
    );
  };
        ...
        {view === 'day' ? (
          <Card className="overflow-hidden">
            <CardContent className="p-0">
              <div className="grid grid-cols-[80px_repeat(3,1fr)] divide-x divide-border">
                {/* Time Column Header */}
                  ...
                  <>
                    ...
                    {mockEmployees.map((employee) => {
                      const appointment = getAppointmentsForDate(currentDate).find(
                        (a) => a.employeeId === employee.id && a.startTime === time
                      );
                      ...
                      return (
                        <div key={`${employee.id}-${time}`} className="border-b p-2 min-h-[80px] relative">
                          {appointment && (
                            ...
                            >
                              <p className="font-semibold truncate">{appointment.client?.name}</p>
